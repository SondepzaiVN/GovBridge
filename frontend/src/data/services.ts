...
const normalizeField = (field: ApiProcedureField): FormField => {
  ...
  const cccdKey = rawCccdKey && cccdKeys.has(rawCccdKey as keyof CCCDInfo)
    ? rawCccdKey as keyof CCCDInfo
    : undefined;
...
const setPublicServices = (services: PublicService[]) => {
  ...
  ROUTE_TO_SERVICE_MAP = Object.fromEntries(
    services.map((service) => [service.route, service]),
  );
};
